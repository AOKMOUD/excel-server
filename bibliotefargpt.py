...
unique_genres = sorted(book_data['Жанр'].dropna().unique().tolist()) if not book_data.empty and "Жанр" in book_data.columns else []

# Главное меню с изображениями (эмодзи)
...
async def show_search_criteria(update: Update, context: CallbackContext) -> None:
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.edit_message_text("🔎 *Выберите критерии для поиска книги:*", reply_markup=reply_markup, parse_mode="Markdown")

# Список жанров с изображениями
async def show_genres(update: Update, context: CallbackContext) -> None:
    query = update.callback_query
    if unique_genres:
        keyboard = [[InlineKeyboardButton(f"📖 {genre}", callback_data=f"genre_{i}")] for i, genre in enumerate(unique_genres)]
        reply_markup = InlineKeyboardMarkup(keyboard)
            ...
            if not book_row.empty:
                ...
                for _, row in book_row.iterrows():
                    for column in required_columns:
                        value = str(row[column]).replace("-", "\\-").replace(".", "\\.").replace("(", "\\(").replace(")", "\\)").replace("!", "\\!")
                        if pd.notnull(value):
                            response += f"*{column}:* {value}\n"
                    response += "\n"
...
async def handle_author_search_input(update: Update, context: CallbackContext) -> None:
    ...
    context.user_data['waiting_for_author_search'] = False
    ...
    if all(column in book_data.columns for column in required_columns):
        try:
            ...
            await update.message.reply_text(f"❌ Ошибка при поиске автора: {str(e)}")
    else:
        await update.message.reply_text("❌ Ошибка: В файле отсутствуют необходимые столбцы.")
...
async def handle_book_description(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        ...
        if 'book_list' in context.user_data and 1 <= book_number <= len(context.user_data['book_list']):
            ...
            if not book_row.empty:
                ...
                for column in book_row.columns:
                    if column != "Название_очищенное" and pd.notnull(book_row[column].values[0]):
                        value = str(book_row[column].values[0]).replace("-", "\\-").replace(".", "\\.").replace("(", "\\(").replace(")", "\\)").replace("!", "\\!")
                        response += f"*{column}:* {value}\n"
                await update.message.reply_text(response, parse_mode="MarkdownV2")
                ...
                await update.message.reply_text(f"❌ Книга с названием '{book_name}' не найдена.")
        else:
            await update.message.reply_text("❌ Ошибка: Неверный номер книги. Попробуйте снова.")
    except ValueError:
        await update.message.reply_text("❌ Ошибка: Введите числовое значение.")
